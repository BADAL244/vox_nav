...
namespace nav2_gps_waypoint_follower_client
{
...
GPSWayPointFollowerClient::loadGPSWaypointsFromYAML()
{
  ...
  for (auto && curr_waypoint : waypoints_vector) {
    try {
      ...
      this->declare_parameter(curr_waypoint, rclcpp::ParameterValue{"0"}, descriptor);
...
int main(int argc, char const * argv[])
{
  ...
  auto gps_waypoint_follower_client_node = std::make_shared
    <nav2_gps_waypoint_follower_client::GPSWayPointFollowerClient>();
